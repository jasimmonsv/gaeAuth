...
class authenticate(): #this function will do all the checks for user info
    
    #ensure username is a string and nothing else
    def userTypeStr(self, username):
        return isinstance(username,str)
        
    #function will validate that username is between 3 and 25 characters long
    def usernameLength(self, username):
        if (username <3) or (username >25):
            return False
        else: return True
        
    def userHasSpecChar(self, username):
        #TODO: search for Special Characters and non-printable characters
        return True
